...
```{r}
initial_data = read_csv("nellaphen_data_cleaned.csv") %>%
...
low_acc_IDs = initial_data %>%
  ...
  filter(id_mean_correct < 0.75) %>% pull (sona_id)

nellaphen_data = initial_data %>%
  filter(!sona_id %in% low_acc_IDs)
```
...
condition_comparison_id = nellaphen_data %>%
  filter(typeoftrial == "target", type == "shared", rt > 200, rt < 1500) %>%
  group_by(sona_id, relatedness, condition) %>%
  summarize(mean_rt = mean(rt))
comparison = lmer(data = condition_comparison_id, mean_rt ~ relatedness * condition + (1|sona_id))
