...
```{r}
initial_sona_data = read_csv("nellaphen_data_cleaned.csv") %>% 
  select(-sona_id)

initial_prolific_data1 = read_csv("prolific-semantic-integration-nellaphen (1).csv") %>% select(-view_history)

initial_prolific_data2 = read_csv("prolific-semantic-integration-nellaphen-2.csv") %>% select(-view_history)
