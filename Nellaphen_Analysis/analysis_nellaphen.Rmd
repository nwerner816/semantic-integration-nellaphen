...
condition_comparison = nellaphen_data %>%
  filter(typeoftrial == "target", type == "shared") %>%
  group_by(relatedness, condition) %>%
  mutate(rt = as.numeric(rt)) %>%
  summarize(mean_rt = mean(rt))

ggplot(data = condition_comparison) +
  geom_col(mapping = aes(x = condition, y = mean_rt, fill = relatedness), position = "dodge") +
  theme_bw()+
...
```{r}
comparison = lm(data = condition_comparison, mean_rt ~ relatedness * condition)
